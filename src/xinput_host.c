...
bool xinputh_open(uint8_t __attribute__((unused)) rhport, uint8_t dev_addr, tusb_desc_interface_t const *desc_itf, uint16_t max_len)
{
...
bool xinputh_xfer_cb(uint8_t dev_addr, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes)
{
    ...
    if (result != XFER_RESULT_SUCCESS)
    {
        ...
        {
            tuh_xinput_report_sent_cb(dev_addr, instance, xid_itf->epout_buf, (uint16_t) xferred_bytes);
        }
        return false;
    }
    ...
    if (dir == TUSB_DIR_IN)
    {
        ...
        else if (xid_itf->type == XBOXONE)
        {
            if (rdata[0] == GIP_CMD_INPUT)
            {
                ...
                //Map the left and right triggers
                pad->bLeftTrigger  = (uint8_t) ((rdata[7] << 8 | rdata[6]) >> 2);
                pad->bRightTrigger = (uint8_t) ((rdata[9] << 8 | rdata[8]) >> 2);
            ...
            else if (rdata[0] == GIP_CMD_VIRTUAL_KEY)
            {
                if (rdata[4] == 0x01 && !(pad->wButtons & XINPUT_GAMEPAD_GUIDE)) {
                    ...
                    xid_itf->new_pad_data = true;
                    pad->wButtons &= (uint16_t) ~XINPUT_GAMEPAD_GUIDE;
                }
            }
